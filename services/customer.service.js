...
const findOne = async ({ params }) => {
    try {
        ...
        else return { type: 'bad', message: `${data.name.toUpperCase()} Customer not exist!` }
    } catch (error) {
        throw error;
    }
};
...
const findAll = async (req) => {
    try {
        ...
        if (data.length) return { type: 'success', message: `Customer found`, data }
...
const update = async ({ params, body }) => {
    try {
        ...
        const data = await CUSTOMERS_MODEL.findByIdAndUpdate(_id, body, { new: true });
        if (data) return { type: 'success', message: `${data.name.toUpperCase()} Customer Updated`, data }
        else return { type: 'bad', message: `Customer not found` }
